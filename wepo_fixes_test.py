...
def run_fixes_tests():
    ...
    try:
        ...
        log_test("Transaction Validation Fixes", False, error=str(e))
        print(f"  ✗ Exception: {str(e)}")
    
    # 2. UTXO Management and Balance Calculation Tests
    try:
        ...
        log_test("UTXO Management and Balance Calculation Fixes", False, error=str(e))
        print(f"  ✗ Exception: {str(e)}")
    
    # 3. Reward Calculations Tests
    try:
        ...
        if mining_info:
            ...
            if miner_wallet:
                ...
                if mine_result:
                    ...
                    # Verify reward matches expected Q1 value (400 WEPO)
                    actual_reward = mine_result.get('reward', 0)
                    
                    if abs(actual_reward - 400.0) < 0.1:
                        print(f"  ✓ Mining reward matches expected Q1 value (400 WEPO)")
                        passed = True
                    else:
                        print(f"  ✗ Mining reward {actual_reward} doesn't match expected Q1 value (400 WEPO)")
                        passed = False
                else:
        ...
        log_test("Reward Calculations Fixes", False, error=str(e))
        print(f"  ✗ Exception: {str(e)}")
    
    # 4. Balance Updates Tests
    try:
                    ...
                    if balance_a_initial > 0:
                        ...
                        if tx_a_to_b and tx_a_to_b.get("status") != "failed":
                            ...
                            if mine_result:
                                ...
                                expected_a_step1 = balance_a_initial - 50.0 - 0.0001  # Subtract transfer and fee
                                expected_b_step1 = 50.0
                                
                                balance_a_step1_correct = abs(balance_a_after_step1 - expected_a_step1) < 1.0
                                balance_b_step1_correct = abs(balance_b_after_step1 - expected_b_step1) < 1.0
                                
                                if balance_a_step1_correct and balance_b_step1_correct:
                                    print("  ✓ Balance updates after step 1 are correct")
                                    ...
                                    if tx_b_to_c and tx_b_to_c.get("status") != "failed":
                                        ...
                                        if mine_result:
                                            ...
                                            # Verify balance updates
                                            expected_b_step2 = balance_b_after_step1 - 25.0 - 0.0001  # Subtract transfer and fee
                                            expected_c_step2 = 25.0
                                            
                                            balance_b_step2_correct = abs(balance_b_after_step2 - expected_b_step2) < 1.0
                                            balance_c_step2_correct = abs(balance_c_after_step2 - expected_c_step2) < 1.0
                                            
                                            if balance_b_step2_correct and balance_c_step2_correct:
                                                print("  ✓ Balance updates after step 2 are correct")
                                                ...
                                                if tx_c_to_a and tx_c_to_a.get("status") != "failed":
                                                    ...
                                                    if mine_result:
                                                        ...
                                                        expected_a_final = balance_a_after_step1 + 10.0
                                                        expected_c_final = balance_c_after_step2 - 10.0 - 0.0001  # Subtract transfer and fee
                                                ...
                                                print("  ✗ Balance updates after step 2 are incorrect")
                                                if not balance_b_step2_correct:
                                                    print(f"  ✗ Wallet B expected: ~{expected_b_step2}, actual: {balance_b_after_step2}")
                                                if not balance_c_step2_correct:
                                                    print(f"  ✗ Wallet C expected: ~{expected_c_step2}, actual: {balance_c_after_step2}")
                                                passed = False
                                        else:
                                    ...
                                    print("  ✗ Balance updates after step 1 are incorrect")
                                    if not balance_a_step1_correct:
                                        print(f"  ✗ Wallet A expected: ~{expected_a_step1}, actual: {balance_a_after_step1}")
                                    if not balance_b_step1_correct:
                                        print(f"  ✗ Wallet B expected: ~{expected_b_step1}, actual: {balance_b_after_step1}")
                                    passed = False
                            else:
        ...
        log_test("Balance Updates Fixes", False, error=str(e))
        print(f"  ✗ Exception: {str(e)}")
