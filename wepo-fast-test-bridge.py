...
# Initialize Community Price Oracle and Dynamic Collateral System (for API compatibility)
...
class WepoFastTestBridge:
    ...
    def setup_cors(self):
        self.app.add_middleware(
            CORSMiddleware,
            allow_origins=[
                "https://130f3a1c-445d-47c5-ac8a-2b468eeb6e1f.preview.emergentagent.com",  # Production frontend
    ...
    def setup_routes(self):
        ...
        # ===== DYNAMIC COLLATERAL SYSTEM ENDPOINTS =====
        
        @self.app.get("/api/collateral/dynamic/masternode")
        async def get_dynamic_masternode_collateral():
            """Get current dynamic masternode collateral requirements based on community DEX price"""
            try:
                # Update price oracle with latest DEX data
                current_price = btc_wepo_pool.get_price()
                if current_price:
                    community_price_oracle.get_wepo_usd_price(current_price)
                
                collateral_info = dynamic_collateral.calculate_masternode_collateral()
                
                return {
                    "success": True,
                    "collateral_system": "dynamic_community_based",
                    "masternode_requirement": collateral_info,
                    "dex_status": {
                        "pool_exists": btc_wepo_pool.total_shares > 0,
                        "current_wepo_per_btc": current_price,
                        "btc_reserve": btc_wepo_pool.btc_reserve,
                        "wepo_reserve": btc_wepo_pool.wepo_reserve
                    },
                    "message": "Dynamic collateral based on community-determined WEPO price"
                }
                
            except Exception as e:
                raise HTTPException(status_code=500, detail=str(e))
        
        @self.app.get("/api/collateral/dynamic/pos")  
        async def get_dynamic_pos_collateral():
            """Get current dynamic PoS staking requirements based on community DEX price"""
            try:
                # Update price oracle with latest DEX data
                current_price = btc_wepo_pool.get_price()
                if current_price:
                ...
                collateral_info = dynamic_collateral.calculate_pos_collateral()
                
                return {
                    "success": True,
                    "collateral_system": "dynamic_community_based",
                    "pos_requirement": collateral_info,
                    "dex_status": {
                        "pool_exists": btc_wepo_pool.total_shares > 0,
                        "current_wepo_per_btc": current_price,
                        "btc_reserve": btc_wepo_pool.btc_reserve,
                    ...
                    "message": "Dynamic PoS staking based on community-determined WEPO price"
                }
                
            except Exception as e:
                raise HTTPException(status_code=500, detail=str(e))
        
        @self.app.get("/api/collateral/dynamic/overview")
        async def get_dynamic_collateral_overview():
            """Get complete overview of dynamic collateral system"""
            try:
                # Update price oracle with latest DEX data
                current_price = btc_wepo_pool.get_price()
                if current_price:
                ...
                masternode_info = dynamic_collateral.calculate_masternode_collateral()
                pos_info = dynamic_collateral.calculate_pos_collateral()
                
                return {
                    "success": True,
                    "system_name": "WEPO Community-Driven Dynamic Collateral",
                    "price_oracle": {
                        "source": "community_dex",
                        "wepo_usd_price": community_price_oracle.get_stable_price(),
                        "btc_reference": community_price_oracle.btc_usd_reference,
                        "last_update": community_price_oracle.last_update.isoformat() if community_price_oracle.last_update else None,
                        "price_history_points": len(community_price_oracle.price_history)
                    },
                    "collateral_requirements": {
                        "masternode": masternode_info,
                        "pos_staking": pos_info
                    },
                    "dex_statistics": {
                        "pool_exists": btc_wepo_pool.total_shares > 0,
                        "current_wepo_per_btc": current_price,
                        "total_volume_btc": btc_wepo_pool.total_volume_btc,
                        "total_swaps": btc_wepo_pool.total_swaps,
                        "liquidity_providers": len(btc_wepo_pool.lp_positions)
                    },
                    "advantages": [
                        "No external oracle manipulation",
                        "Community-determined fair pricing",
                        "Automatic adjustment to market conditions",
                        "Maintains accessibility as WEPO appreciates",
                        "Governed by halving-cycle democracy"
                    ]
                }
        ...
        @self.app.post("/api/collateral/dynamic/update-btc-reference")
        async def update_btc_reference_price(request: dict):
            """Update BTC reference price (governance-controlled)"""
            try:
                new_btc_price = float(request.get("btc_price", 0))
                governance_signature = request.get("governance_signature", "")
                
                if new_btc_price <= 0:
                    raise HTTPException(status_code=400, detail="Invalid BTC price")
                
                # TODO: Add governance signature verification here
                # For now, simple validation
                if not governance_signature:
                    raise HTTPException(status_code=403, detail="Governance signature required")
                
                old_price = community_price_oracle.btc_usd_reference
                community_price_oracle.update_btc_reference(new_btc_price)
                
                return {
                    "success": True,
                    "btc_reference_updated": True,
                    "old_price": old_price,
                    "new_price": new_btc_price,
                    "update_time": datetime.now().isoformat(),
                    "message": "BTC reference price updated via governance"
                }
