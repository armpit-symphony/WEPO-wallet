...
const SendWepo = ({ onClose, isPreGenesis = false }) => {
  ...
  const handleSend = async () => {
    if (isPreGenesis) return; // hard block
  ...
  return (
    <div className="space-y-6">
      {isPreGenesis && (
        <PreGenesisBanner message="Sending transactions is disabled until genesis. Please check back at launch." />
      )}
      ...
      <div className="space-y-4">
        <div>
          ...
          <input type="text" name="toAddress" value={formData.toAddress} onChange={handleInputChange}
            ...
            placeholder="wepo1..." required disabled={isPreGenesis} />
        </div>
        <div>
          ...
          <div className="relative">
            <input type="number" name="amount" value={formData.amount} onChange={handleInputChange}
              ...
              placeholder="0.0000" step="0.0001" min="0" required disabled={isPreGenesis} />
        ...
        <div>
          ...
          <div className="relative">
            <input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleInputChange}
              ...
              placeholder="Enter your wallet password" required disabled={isPreGenesis} />
            <button type="button" onClick={() => setShowPassword(!showPassword)}
              className="absolute right-3 top-3 text-gray-400 hover:text-purple-400" disabled={isPreGenesis}>
      ...
      {success && (
        <div className="bg-green-900/50 border border-green-500 rounded-lg p-3 text-green-200 text-sm">{success}</div>
      )}
      ...
      <div className="bg-gray-700/30 rounded-lg p-4">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between border-t border-gray-600 pt-2">
            ...
            <span className="text-white font-medium">{formData.amount ? (parseFloat(formData.amount) + 0.0001).toFixed(4) : '0.0001'} WEPO</span>
          </div>
        </div>
      </div>
